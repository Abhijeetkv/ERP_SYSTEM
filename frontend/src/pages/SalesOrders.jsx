...
const SalesOrders = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        console.error('Sales orders load failed', error);
  ...
  const columns = [
    ...
    {
      header: 'Total Qty', accessor: (item) =>
        item.lines?.reduce((sum, line) => sum + (line.qty || 0), 0) ?? 0,
    },
    {
      header: 'Total Value',
      accessor: (item) => {
        const total = item.lines?.reduce(
          (sum, line) => sum + (Number(line.unitPrice || 0) * (line.qty || 0)),
          0
        );
    ...
    <div className="space-y-6">
      <div className="rounded-3xl border border-gray-200 bg-white px-6 py-6 shadow-sm">
        <div>
          ...
          <p className="text-sm text-slate-500">Review sales orders and delivery status.</p>
        </div>
      </div>
      {loading ? (
